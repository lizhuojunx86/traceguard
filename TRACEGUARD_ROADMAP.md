...
- **v1.0.0 已发布(freeze-flip,零功能变更)**:下方 2026-06-28 更新定义的三个条件已逐条兑现——① SPEC §3–5 MUST 全实现并强制(含不变量 4,0.8.0)+ ② 29 符号公开面 curated 并由 required `contract-guard` CI 机制冻结(0.8.0/0.9.0)+ ③ fail-open(0.8.0 B1)。SPEC 状态行随之从 Draft v0.3 翻正为 **v1.0**(版本从此跟随包 major,中文版继续权威)。
- **Phase 0 验收 #7 已达成(2026-06-29,修正下文"至今未满足"的过时表述)**:huadian(经 guardian 桥,126 条)与 quant_alpha_v2(手工 span,252 条、抓到 206 条真实 look-ahead 违规)均为真实外部 trace。
- **soak 证据**:0.9.0 发布后冻结面静置约两周(2026-06-29 → 07-12),期间 18 个提交全部限于契约外的 routing_audit 实验模块,零 hotfix、零契约面改动。
- **后续路线(post-1.0)**:按 2026-07-12 战略评估,方向为"证据层而非控制层"——hash chain / append-only 审计守卫优先,干预型能力(审批门/策略门)仅在真实用户拉动时以 §6.6 off-surface opt-in extra 形态考虑。
...
| `traceguard` | 新 SDK + 存储,服务未来接入 | `v0.2.0`(本路线第一个 release) | ~~Phase 2 完成时冻结 v1.0.0~~ → 见顶部 2026-06-28 状态更新(改为契约核心 + 冻面 + fail-open)→ **v1.0.0 已冻结 (2026-07-12)** |
