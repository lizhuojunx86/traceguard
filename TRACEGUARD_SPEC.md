...
## 附录 B2: 派生表的修正语义(目标态,**尚未实现**)

记录一个已确认的设计缺陷与其目标形态。**当前不实现——这是记录方向,不是排期。**
适用范围是 contract-external 的 `routing_audit.*` 派生表,不构成本规范的 MUST。

### 现状与后果

`routing_decisions` 以 `decision_id` 为主键做 upsert,**人工输入与派生判定同行**:

| 类别 | 列 |
|---|---|
| 人工 | `reason`、`outcome`、`source` |
| 派生 | `expected_tier`、`expected_model`、`actual_model`、`actual_tier`、`deviation`、`cost_usd`、`n_traces`、`ts`、`project`、`task_type`、`batch_id` |

在这个形状下,任何"保护人工输入"的实现都**必然连带冻结派生数据**——两者在同一行,
唯一的保护粒度是整行。2026-08-08 实测:`generate` 对 `source='manual'` 的行整行
`continue`,于是 96 条被人工审阅过的偏差,其 `deviation` 判定停在 2026-07-04 的批次,
**永久豁免于此后任何一次路由策略修订**,而它们占当时全部偏差的 60%。

后果不是成本漂移,是**审计信号对策略停止响应**:标注一条偏差,等于冻住"它是偏差"
这个判断,而被标注的恰好是最被认真看过的那些行。

该缺陷仅因一处打印值($1,945.1544)与存储值($1,945.2578)不一致才暴露,
而那次比对只存在于给人看的输出里,没有进入任何机器可读的位置。

**已做的前向修复**(2026-08-08):显式声明 `MANUAL_COLUMNS` / `DERIVED_COLUMNS` /
`IDENTITY_COLUMNS` 三个集合,`generate` 重算 manual 行的派生列而保留人工列;
测试断言三集合对表的列**恰好划分**,新增未归类的列直接失败。这关闭了前向风险,
但没有改变"覆盖式修正"这个形状本身。

### 目标形态

1. **派生判定 append-only**:修正靠追加新版本,不靠覆盖。每次重建写入新行并带
   classifier / policy 版本号,旧行保留。
2. **视图解析当前判定**,且**最顺手的名字必须指向安全的那个**:原表改名带 `_raw`
   后缀,`routing_decisions` 这个名字指向"当前有效判定"的视图;想读到被取代的历史
   必须刻意输入 `_raw`。
3. **人工注解迁到独立表**,挂在判定的身份(`decision_id`)上,而不是作为派生行的列。
   这样"保护人工输入"与"刷新派生数据"在物理上不再互相牵制,不依赖任何一方的纪律。

### 依据

同一操作者的 pit-archive 已经解过这个问题,形状可直接借用:
`pit.ingestion_log_raw` 是表且 append-only,`pit.ingestion_log` 是视图且是默认名,
修正靠 classifier 版本号叠加而非覆盖。其结论是:

> 靠纪律维持的正确性会腐化,靠命名维持的不会。

### 为什么现在不做

需要 schema 迁移与历史回填,而列拆分已经关闭了前向风险——派生列每次重建都会更新,
不再存在永久孤岛。剩下的是"修正即覆盖"这一形状问题,不是正在流血的伤口。

### 现状只有一半

`routing_decisions_rebuild_log.jsonl` 已经是这套模式的**一半**:每次重建记录
who / when / batch / 重建前后 / 逐行变更归因,对应 pit-archive 的 `rebuild_log`。
缺的是 append-only 那一半——判定本身仍然被覆盖,只有"曾经重建过"这件事被保留。
读到这里的人若发现只有一半,原因在此。
...
「变了吗」通常确定且廉价。「变成了什么」昂贵且可能失败。把两件事绑在一起,
等于让贵的那个的失败吃掉便宜那个的确定性。

- **反面**:pit-archive 的 DTOP 保留期看门狗。若把「预期的 floor 还在不在」和
  「新的 floor 在哪」写成一个流程,探测预算耗尽就会连「floor 没了」这个确定结论
  一起丢掉。实际实现把两者拆开:前者是一次确定性探测、永远发出告警;后者
  best-effort,可以带着「未测得」返回。
- **正面**:`derived_drift` 看门狗只回答「N 行的存储值与当场重算不一致」,
  不负责算出正确值。便宜、确定、每次都能跑完。

### B3.2 任何「与上次比较」的检查必须回写本次观测值

否则比较的锚点是一个固定的历史点,漂移会累积成慢性假警报。**而慢性假警报不会
让人怀疑代码,只会让人不看告警。**

- **实例**:task-tag 新鲜度看门狗比较「最新 tag 的 `ts_start`」与「最新 trace 的
  `invoked_at`」,两端每次运行都重新读取。若把基准写死成上线时的日期,五周后它
  会天天报警,而报警内容永远相同。
- **同一终点,另一条路**:`missing_price` 告警存在、触发了十次、有通过的测试,
  但只写进 stdout,而机器可读的 run log 记录了十八次空 `warnings`。**告警响了
  没人看,和告警天天响没人看,终点是同一个。**

### B3.3 懒惰路径必须等于正确路径

靠纪律维持的正确性会腐化,靠命名维持的不会。最顺手的那条路必须是安全的那条。

- **实例**:pit-archive 的 `pit.ingestion_log` 是**视图**且是默认名(当前有效判定),
  `pit.ingestion_log_raw` 是**表**,读它必须刻意多打 `_raw`。`checks.py` 曾经查了
  raw 表,把 248 条已撤回的判定复活成实时告警——之后靠改名而不是靠提醒解决。
- **实例**:`.gitignore` 逐个扩展名列举冲突副本(`* 2.py` / `* 2.md` / ...),
  于是 `" 3."` 那批和 `.json`/`.csv` 那批一直漏在外面。改成按**形状**匹配
  (`* 2.*` / `* 3.*`)之后,新出现的扩展名不需要任何人记得去补。

### B3.4 假阴性比假阳性危险

只测「不会误报」是把看门狗调成哑巴的经典方式。**每一条告警都必须有一个证明它
会响的测试**,而不只是证明它不乱响的测试。

- **实例**:task-tag 看门狗最初挂在写入循环之前,首次 ingest 时 traces 表还空着,
  `total == 0` 直接 return——它把自己静音了。被一条真阳性测试当场抓出。
- **实例**:`test_dry_run_writes_nothing` 断言 `inserted == 0`。那个 0 不是预期值,
  是 dry-run 提前 return 导致计数器**结构性恒为零**。测试一直绿着,而 dry-run
  的全部意义(写库前看见会改什么)从未生效。
- **最纯的一例**:一条验收命令写成 `grep -rn ... --include=*.py .`,未加引号。
  bash 下透传、正常执行;zsh 下 `nomatch` 直接中止,grep 根本没跑,而空输出被
  读成「干净」。**一条检查因为跑不起来而报告了成功。** 推论:优先用能正向看见
  的结果(计数、行、退出码),而不是「应无输出」。

### B3.5 不要把「当下为真」说成「本来如此」(新增 2026-08-08)

原四条之外。这一条在同一次审计里连犯两次,代价是两个进了代码库的错数字。

一个事实在**当前系统状态下**成立,不等于它是**系统的性质**。前者会被下一次
提交推翻,后者不会。写下结论时必须能回答:这句话依赖哪些当下的条件?

- **实例**:「缓存倍率缺陷让全库少算 $1,393.49 / 27,130 行」。实际是
  $88.75 / 3,918 行。差别在于:我测的是「一个瞎读平铺的 reader **会**算出什么」,
