...
**这两个数字可以自己核。** 支撑它们的 2,163 条记录已随仓库发布,在
[`analysis/data/`](../../analysis/data/)。数值本身被换成了带密钥的摘要和分桶,
所以厂商数据一点没有外流。跑 `python analysis/eps_revision.py` 会打印两个比率、
置信区间和一张阈值敏感性表。"翻转一个可交易的决策"的精确定义,以及这个测量
可能哪里不对的十二条清单,都在[方法论文档](../eps-revision-methodology.md)里。
另有一份更晚开始、universe 更宽但观察期更短的捕获,给出 18.6% / 4.6%,一并发布,
差异原因在同一份文档中说明。
