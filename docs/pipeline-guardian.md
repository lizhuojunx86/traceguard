...
> **Status: frozen (maintenance only).** Pipeline Guardian (package
> `pipeline-guardian`, import name `guardian`) is the original
> checkpoint-validation system in this repository. It receives bugfixes only;
> its public API (the 4 symbols listed under
> [Stability for Downstream Integrators](#stability-for-downstream-integrators))
> stays stable for existing integrators. All new development happens in the
> **traceguard SDK** — see the [repository README](../README.md).
...
> Architecture details and design rationale: [Pipeline Guardian Specification](Pipeline_Guardian_Specification.html)
...
For a complete architecture walkthrough, see [Pipeline Guardian Specification](Pipeline_Guardian_Specification.html).
