...
2026-08-22 · companion to [`CONFORMANCE.md`](../CONFORMANCE.md) (Claude Code, I-1..I-11)
...
- **tokscale [#1162](https://github.com/junhoyeo/tokscale/pull/1162)** — four
  ...
  arm reverted, three of the four fail. Open as of today.
...
- **I-10** — [viberank#125](https://github.com/sculptdotfun/viberank/issues/125),
  open with no reply. It did become the designated home for a second bug:
  nikshepsvn wrote that same-machine self-correction "needs care against the
  drift guard's reason for existing, so it'll be considered with #125's
  per-(machine, agent) contribution keying rather than rushed."
- **D-1..D-5 upstream** — deepseek-harness master still has no
  `compaction/summary` branch, does not read `llm/retry`, and reports
  `stateVersion: 1`. Checked 2026-08-22. External PRs are closed by
  CONTRIBUTING, so the verified fix cannot move on its own.
